...
if (isset($_GET["mode"])&&$_GET["mode"]=="search") {
    ...
    $sql="SELECT * from `review` where $find like '%$q_search%' order by num desc;";
} else {
    $sql="SELECT * from `review` order by group_num desc, ord asc;";
}
...
<html lang="ko" dir="ltr">
  ...
  <body>
    ...
    <section>
      <div id="review_list_div_main">
         <div id="review_title">
           <h3>후기 : 매달 후기를 남겨주신 분을 추첨하여 할인 쿠폰을 드립니다 ^0^</h3>
         </div>
         <form name="board_form" action="review_list.php?mode=search" method="post">
           <div id="list_search">
           ...
           <?php
            for ($i = $start; $i < $start+SCALE && $i<$total_record; $i++) {
              ...
              <div id="list_item">
                ...
                <div id="list_item2">
                    <a href="./review_view.php?num=<?=$num?>&page=<?=$page?>&hit=<?=$hit+1?>"><?=$space.$subject?></a>
                </div>
          ...
          <div id="page_button">
            ...
            <?php
              if ($total_page>=2 && $page >=2) {
                  $new_page = $page-1;
                  echo "<li><a href='review_list.php?page=$new_page'>◀ 이전&nbsp;&nbsp;</a></li>";
              } else {
                  echo "<li>&nbsp;</li>";
              }
              ...
              for ($i=1; $i <= $total_page ; $i++) {
                  if ($page == $i) {
                      ...
                      echo "<li><a href='review_list.php?page=$i'> $i </a></li>";
                  }
              }

              if ($total_page>=2 && $page != $total_page) {
                  $new_page = $page+1;
                  echo "<li><a href='review_list.php?page=$new_page'>&nbsp;&nbsp;다음 ▶</a></li>";
              } else {
                  echo "<li>&nbsp;</li>";
              }
            ?>
