// 모든 문서가 다 로딩이 되면 자동으로 실행해주는 함수(document.ready랑 같다)
$(function () {
  var slideshow = $('.slideshow'),
      slideshowImg = slideshow.find('#slideshow_img'),
      slides = slideshowImg.find('a'),//슬라이드 네개니까 배열로 들어옴
      slidesCount = slides.length,
      nav = slideshow.find('#slideshow_nav'),
      prev = nav.find('.prev'),
      next = nav.find('.next'),
      currentIndex = 0, //현재 슬라이드를 첫번째 화면으로 세팅해주는 변수값
      ...
      //가로로 슬라이드 배치
      //slides[0]~slide[3] 왼쪽 0% -> 100%씩 증가
